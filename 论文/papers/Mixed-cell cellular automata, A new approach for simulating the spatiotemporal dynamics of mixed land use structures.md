...
#### 2.2.2 混合元胞内的土地利用竞争
在得到总发展概率 $TP_{k,l}^t$ 后，采用轮盘赌选择机制来解决模拟过程中亚元胞尺度的土地利用竞争问题（图 4）。首先，利用下式对各种土地利用成分的总发展概率进行归一化处理：
...
第一步是从模拟和实际的土地利用图中随机选取若干个元胞。第二步是根据以下公式计算各土地利用分量（$u$ 或 $v$）对亚像元混淆矩阵中对角线元素（$u=v$）的一致性，以及各土地利用分量对各样本元胞 $i$ 的非对角线元素（$u\not ={v}$）的不一致性。
